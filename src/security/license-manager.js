...
export class LicenseManager {
  ...
  async verifyEcdsaSignature(payload, signatureStr) {
    try {
      ...
      if (cryptoKey && cryptoObj) {
        ...
        const ecResult = await cryptoObj.verify(
          LICENSE_ALGORITHM,
          cryptoKey,
          sigBytes,
          dataBytes
        );
        if (ecResult) return true;
      }

      // ECDSA failed or unavailable — try HMAC-SHA256 fallback.
      // The HTML license generator (license-generator-app.html) produces
      // 8-char HMAC signatures instead of ECDSA. Accept both schemes.
      return await this.verifyHmacSignature(payload, signatureStr);
    } catch (e) {
      return await this.verifyHmacSignature(payload, signatureStr);
    }
  }
  ...
  /**
   * HMAC-SHA256 fallback verification for keys generated by the HTML
   * admin tool (license-generator-app.html). Uses the same secret salt
   * and truncation as the HTML generator.
   */
  async verifyHmacSignature(payload, signatureStr) {
    try {
      const HMAC_SECRET = "MK_ELITE_SECURE_SALT_2026_STAGE_PRO";
      ...
      if (cryptoObj) {
        ...
        const sig = await cryptoObj.sign("HMAC", key, enc.encode(payload));
        const hex = Array.from(new Uint8Array(sig))
          .map(b => b.toString(16).padStart(2, "0"))
          .join("")
          .substring(0, 8)
          .toUpperCase();
        return hex === signatureStr;
      }

      // Web Crypto unavailable — FNV-1a fallback matching the HTML generator's catch path
      let hash = 0x811c9dc5;
      const combined = payload + HMAC_SECRET;
