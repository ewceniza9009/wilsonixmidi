...
export class ChordPadsUI {
  constructor(containerId) {
    ...
    // Real-time Chord Namer state: everything below is UI-side bookkeeping on
    // the main thread (event-driven, no audio-path involvement).
    ...
    this._richChord = null; // fullest voicing of the current execution (sealed)
    this._history = []; // executed chords, newest last, trimmed to MAX_HISTORY
  ...
  /**
   * Live chord naming works for ANY real-time input source (qwerty keyboard,
   * on-screen keyboard, MIDI hardware, chord pads) because every source funnels
   * through multiLayerEngine.noteOn/noteOff. Scheduled playback (looper,
   * arpeggiator, demo) passes when > 0 and is deliberately excluded so the
   * namer only reflects what the user actually presses.
   */
  _subscribeLiveNotes() {
